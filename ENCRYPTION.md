...
export const EncryptionConfig = {
  enabled: true,
  keyVersion: 'v1',
  publicKeyBase64: '<BASE64_ENCODED_PUBLIC_KEY>',  // 实际公钥
...
encryption:
  ...
  keys:
    - version: v1
      privateKey: <BASE64_ENCODED_PRIVATE_KEY>  # 从安全存储获取
```
...
**公钥**: 存储在前端配置文件 `encryption.config.ts` 中（Base64 编码）

**私钥**: 存储在后端 `application.yml` 中（Base64 编码，仅服务器可访问）

⚠️ **安全提示**: 
- 私钥绝不应出现在文档、代码仓库或日志中
- 使用环境变量或密钥管理服务（如 AWS Secrets Manager）存储私钥
- 定期轮换密钥
