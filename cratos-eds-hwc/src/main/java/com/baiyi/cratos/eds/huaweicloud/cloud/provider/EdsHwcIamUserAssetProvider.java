...
public class EdsHwcIamUserAssetProvider extends BaseEdsAssetProvider<EdsConfigs.Hwc, KeystoneListUsersResult> {

    public EdsHwcIamUserAssetProvider(EdsAssetProviderContext context) {
        super(context);
    }

    @Override
    protected List<KeystoneListUsersResult> listEntities(
            ExternalDataSourceInstance<EdsConfigs.Hwc> instance) throws EdsQueryEntitiesException {
        EdsConfigs.Hwc huaweicloud = instance.getConfig();
        try {
    ...
    protected List<EdsAssetIndex> buildIndexes(ExternalDataSourceInstance<EdsConfigs.Hwc> instance,
        ...
        // accessKeys
        try {
            List<Credentials> accessKeys = HwcIamRepo.listAccessKeys(instance.getConfig()
                    .getRegionId(), instance.getConfig(), entity.getId());
