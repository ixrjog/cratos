...
import com.baiyi.cratos.eds.jenkins.sdk.client.JenkinsHttpClient;
...
public class JenkinsServer implements Closeable {
    ...
    public Map<String, Job> getJobs(FolderJob folder, String view) throws IOException {
        ...
        if (view != null) {
            path = Joiner.on("")
                    .join(path, "view/", EncodingUtils.encode(view), "/");
            viewClass = View.class;
        }
        List<Job> jobs = client.get(path, viewClass)
                .getJobs();
    ...
    public Map<String, View> getViews(FolderJob folder) throws IOException {
        ...
        List<View> views = client.get(UrlUtils.toBaseUrl(folder) + "?tree=views[name,url,jobs[name,url]]",
                        MainView.class)
                .getViews();
     ...
     * @throws IOException in case of an error.
     */
    public JenkinsServer createJob(FolderJob folder, String jobName, String jobXml,
                                   Boolean crumbFlag) throws IOException {
        client.post_xml(UrlUtils.toBaseUrl(folder) + "createItem?name=" + EncodingUtils.formParameter(jobName), jobXml,
                crumbFlag);
        return this;
    }
     ...
     * @throws IOException in case of an error.
     */
    public JenkinsServer createView(FolderJob folder, String viewName, String viewXml,
                                    Boolean crumbFlag) throws IOException {
        client.post_xml(UrlUtils.toBaseUrl(folder) + "createView?name=" + EncodingUtils.formParameter(viewName),
                viewXml, crumbFlag);
        return this;
    }
    ...
    public Map<String, Computer> getComputers() throws IOException {
        List<Computer> computers = client.get("computer/", Computer.class)
                .getComputers();
        return computers.stream()
                .map(SET_CLIENT(this.client))
                .collect(Collectors.toMap(s -> s.getDisplayName()
    ...
    public JenkinsServer updateView(FolderJob folder, String viewName, String viewXml) throws IOException {
        client.post_xml(UrlUtils.toBaseUrl(folder) + "view/" + EncodingUtils.encode(viewName) + "/config.xml", viewXml,
                true);
        return this;
    }

    public JenkinsServer updateView(FolderJob folder, String viewName, String viewXml,
                                    boolean crumbFlag) throws IOException {
        client.post_xml(UrlUtils.toBaseUrl(folder) + "view/" + EncodingUtils.encode(viewName) + "/config.xml", viewXml,
                crumbFlag);
        return this;
     ...
     * @throws IOException in case of an error.
     */
    public JenkinsServer updateJob(FolderJob folder, String jobName, String jobXml,
                                   boolean crumbFlag) throws IOException {
     ...
     */
    public JenkinsServer addStringParam(String jobName, String name, String description,
                                        String defaultValue) throws IOException, JAXBException, DocumentException {
        String jobXml = this.getJobXml(jobName);
        JobConfiguration jobConf = new JobConfiguration(jobXml);
        jobXml = jobConf.addStringParam(name, description, defaultValue)
                .asXml();
     ...
     * @throws IOException In case of a failure.
     */
    public JenkinsServer renameJob(FolderJob folder, String oldJobName, String newJobName,
                                   Boolean crumbFlag) throws IOException {
        client.post(UrlUtils.toJobBaseUrl(folder, oldJobName) + "/doRename?newName=" + EncodingUtils.formParameter(
                newJobName), crumbFlag);
        return this;
    }
