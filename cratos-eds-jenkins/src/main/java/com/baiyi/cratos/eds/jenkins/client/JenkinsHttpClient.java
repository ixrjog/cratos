...
import com.baiyi.cratos.eds.jenkins.client.util.RequestReleasingInputStream;
import com.baiyi.cratos.eds.jenkins.client.util.ResponseUtils;
import com.baiyi.cratos.eds.jenkins.client.util.UrlUtils;
import com.baiyi.cratos.eds.jenkins.client.validator.HttpResponseValidator;
import com.baiyi.cratos.eds.jenkins.model.BaseModel;
import com.baiyi.cratos.eds.jenkins.model.Crumb;
import com.baiyi.cratos.eds.jenkins.model.ExtractHeader;
...
public class JenkinsHttpClient implements JenkinsHttpConnection {
    ...
    public String get(String path) throws IOException {
        ...
        HttpResponse response = client.execute(getMethod, localContext);
        jenkinsVersion = ResponseUtils.getJenkinsVersion(response);
        log.debug("get({}), version={}, responseCode={}", path, this.jenkinsVersion, response.getStatusLine()
                .getStatusCode());
        try {
            httpResponseValidator.validateResponse(response);
    ...
    public InputStream getFile(URI path) throws IOException {
        ...
        jenkinsVersion = ResponseUtils.getJenkinsVersion(response);
        httpResponseValidator.validateResponse(response);
        return new RequestReleasingInputStream(response.getEntity()
                .getContent(), getMethod);
    }
     ...
     */
    @Override
    public <R extends BaseModel, D> R post(String path, D data, Class<R> cls, Map<String, File> fileParams,
        ...
        try {
            ...
            if (cls != null) {
                ...
                if (cls.equals(ExtractHeader.class)) {
                    ...
                    location.setLocation(response.getFirstHeader("Location")
                            .getValue());
    ...
    public void post_form(String path, Map<String, String> data, boolean crumbFlag) throws IOException {
        ...
        if (data != null) {
            ...
            request = new HttpPost(UrlUtils.toNoApiUri(uri, context, path) + Joiner.on("&")
                    .join(queryParams));
            request.setEntity(stringEntity);
        } else {
     ...
     */
    @Override
    public String post_text(String path, String textData, ContentType contentType,
                            boolean crumbFlag) throws IOException {
        HttpPost request = new HttpPost(UrlUtils.toJsonApiUri(uri, context, path));
        handleCrumbFlag(crumbFlag, request);
     ...
     */
    protected static HttpClientBuilder addAuthentication(final HttpClientBuilder builder, final URI uri,
