package com.baiyi.cratos.eds.ldap.util;

import com.baiyi.cratos.eds.core.config.EdsConfigs;
import com.baiyi.cratos.eds.ldap.model.LdapGroup;
import com.baiyi.cratos.eds.ldap.model.LdapPerson;
import com.google.common.base.Joiner;
import lombok.NoArgsConstructor;
import org.apache.commons.lang3.StringUtils;

import static lombok.AccessLevel.PRIVATE;

/**
 * @Author baiyi
 * @Date 2024/3/8 10:26
 * @Version 1.0
 */
@NoArgsConstructor(access = PRIVATE)
public final class LdapUtils {

    public static String toUserRDN(EdsConfigs.Ldap ldapConfig, LdapPerson.Person person) {
        return Joiner.on("=")
                .join(ldapConfig.getUser()
                        .getId(), person.getUsername());
    }

    public static String toGroupRDN(EdsConfigs.Ldap ldapConfig, LdapGroup.Group group) {
        return Joiner.on("=")
                .join(ldapConfig.getUser()
                        .getId(), group.getGroupName());
    }

    public static String toUserDN(EdsConfigs.Ldap ldapConfig, LdapPerson.Person person) {
        String rdn = toUserRDN(ldapConfig, person);
        return Joiner.on(",")
                .join(rdn, ldapConfig.getUser()
                        .getDn());
    }

    public static String toGroupRDN(EdsConfigs.Ldap ldapConfig, String groupName) {
        return Joiner.on("=")
                .join(ldapConfig.getGroup()
                        .getId(), groupName);
    }

    public static String toGroupDN(EdsConfigs.Ldap ldapConfig, String groupName) {
        if (groupName.endsWith(ldapConfig.getBase())) {
            return StringUtils.removeEnd(groupName, "," + ldapConfig.getBase());
        }
        String rdn = toGroupRDN(ldapConfig, groupName);
        return Joiner.on(",")
                .join(rdn, ldapConfig.getGroup()
                        .getDn());
    }

}